...
export default function JazzCashPaymentScreen() {
    ...
    const handlePayment = async () => {
        if (mobileNumber.length < 11) {
            Alert.alert('Error', 'Please enter a valid JazzCash mobile number');
            return;
        }
        ...
        try {
            const result = await processPayment('jazzcash', total, { mobileNumber });
            ...
            if (result.success) {
                ...
                setProcessing(false);
                Alert.alert('Payment Failed', result.error || 'JazzCash transaction failed.');
            }
        } catch (error) {
            setProcessing(false);
