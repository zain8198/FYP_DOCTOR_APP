...
export default function NotificationsScreen() {
    ...
    const handlePress = async (item: any) => {
        ...
        try {
            // Mark as read
            const itemRef = ref(db, `users/${userId}/notifications/${item.id}`);
            await update(itemRef, { read: true });

            // Navigate to chat if data exists
            if (item.data?.chatId) {
    ...
    const handleDelete = async (id: string) => {
        ...
        if (!userId) return;
        try {
            const itemRef = ref(db, `users/${userId}/notifications/${id}`);
    ...
    const renderItem = ({ item }: { item: any }) => (
        ...
        >
            <View style={[styles.iconBox, { backgroundColor: item.type === 'message' ? '#E3F2FD' : '#FFF3E0' }]}>
                <Ionicons
                    name={item.type === 'message' ? "chatbubble-ellipses" : "notifications"}
                    size={24}
